...
Initially IDA recognizes `pdev` as ObjectClass (which if this was C++ it would have been the base class), however if we change it's type to `PCIDeviceClass` which it should be after the cast, we will see the properties this PCI device is being registered with and 2 callbacks. Uninit is irrelevant to us as it just cleans up after the device has been unloaded. Moving on to the second callback `pci_hitb_realize`, this is where the memory for the MMIO is allocated, mmio ops and the thread that performs the DMA.
...
Moving on to mmio ops we have `hitb_mmio_read` which can get us any relevant HitbState properties. `hitb_mmio_write` changes the state of the device and send commands to the DMA thread. I will condense the functionality of it to the relevant commands.
